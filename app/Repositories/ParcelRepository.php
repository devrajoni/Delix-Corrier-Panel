...
class ParcelRepository implements ParcelInterface {
    ...
    public function chargeDetails($request)
    {
        ...
        if($request->merchant == "" || $request->weight == "" || $request->parcel_type == ""){
            ...
            elseif($request->parcel_type == "sub_city"):
                $location            = 'sub_city';
            elseif($request->parcel_type == "sub_urban_area"):
            ...
            foreach ($system_charge as $charge) {
                if ($charge['weight'] == $request->weight && isset($charge[$request->parcel_type])) {
                    $foundCharge           = $charge[$request->parcel_type];
                    break;
                }
            }

            if (data_get($merchant->charges, $request->weight . '.' . $request->parcel_type)) {
    ...
    public function store($request)
    {
         ...
         try{
            ...
            $parcel->pickup_shop_phone_number    = $request->shop_phone_number ?? $shop->shop_phone_number;
            $parcel->pickup_address              = $request->shop_address ?? $shop->address;
    ...
    public function assignPickupMan($request)
    {
        ...
        try{
            ...
            if ($request->notify_pickup_man == 'notify'):
                ...
                $this->test($sms_body, $parcel->pickupMan->phone_number, 'notify_pickup_man', env('PROVIDER'));
                ...
                $this->sendNotification($title, $users, $details, $permissions, 'success', $parcel->id, '');
    ...
    public function assignDeliveryMan($request)
    {
        ...
        try{
            ...
            if($request->delivery_man):
                $details                    = 'A pickup has assigned to you';
                $users                      = User::where('user_type', operator: 'delivery')
                                                ->whereHas('deliveryMan', function($query) use($request) {
                                                    $query->where('id', $request->delivery_man);
                                                })
                                                ->get();

                $title                      = 'A delivery has assigned to you';

                $permissions                = ['delivery_parcel_read'];
         ...
         try{
            ...
            if($status == 'cancel'){
                ...
                if($sms_template->sms_to_customer):
                    ...
                    $this->test($sms_body, $parcel->customer_phone_number, 'delivery_confirm_otp', $sms_template->masking, env('PROVIDER'));
                endif;
    ...
    public function deliveryReverse($request)
    {
        ...
        try{
            ...
            if (($previous_status == 'partially-delivered' || $previous_status == 'returned-to-warehouse' || $previous_status == 'return-assigned-to-merchant'
           ...
           //if previous delivery reverse, reverse that also
            ...
            elseif ($previous_status == 'delivery-assigned' || $previous_status == 're-schedule-delivery'):
                ...
                if ($request->status == 'transferred-received-by-branch' || $request->status == 'transferred-to-branch' ||
    ...
    public function uptoTransfer($request)
    {
        ...
        if ($request->status == 'transferred-to-branch'||
            $request->status == 'received' || $request->status == 'received-by-pickup-man' ||
            $request->status == 'pickup-assigned' || $request->status == 'pending'):
        ...
        $pickup_person   = DeliveryMan::where('id',$parcel->pickup_man_id)->first();
         ...
         if (!blank($sms_template)):
             if($sms_template->sms_to_merchant):
                 ...
                 //send sms
                 $this->test($sms_body, $parcel->merchant->phone_number, $title, env('PROVIDER'), $sms_template->masking);
             endif;
             //merchant sms end
         endif;
         ...
         if ($this->checkLocation($parcel, $title)):
             ...
             if (!blank($customer_sms_template)):
                 if($customer_sms_template->sms_to_customer):
                     ...
                     $sms_body = str_replace('{short_url}', @$parcel->short_url, $sms_body);
                     //send sms
                     $this->test($sms_body, $parcel->customer_phone_number, $title, env('PROVIDER'), $customer_sms_template->masking);
                 endif;
                 //customer sms end
             endif;
         endif;
