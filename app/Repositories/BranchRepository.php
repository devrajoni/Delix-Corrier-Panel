...
class BranchRepository implements BranchInterface {
    ...
    public function delete($id)
    {
        ...
        try{
            ...
            if ($branch->is_default) {
                DB::rollback();
                return $this->responseWithError(__('default_branch_can_not_be_possible_to_delete'), []);
            }
    ...
    public function statusChange($request)
    {
        ...
        try {
            ...
            // dd($row->default && $row->status == StatusEnum::ACTIVE);
            if ($row->default && $row->status == StatusEnum::ACTIVE) {
                DB::rollback();
                return $this->responseWithError(__('Cannot deactivate the default branch'), []);
            }
