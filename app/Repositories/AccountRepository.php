...
class AccountRepository implements AccountInterface {
    ...
    //             endif;
    //         else:
    //             $return_charge     = $parcel->total_delivery_charge;
    //         endif;
    ...
    //         $merchant_account->type                 = 'expense';
    //         $merchant_account->details              = 'parcel_return_fee_to_greenx_as_parcel_returned';
    //         $merchant_account->save();
    ...
    //         $merchant_account->amount       = floor($total_delivery_charge);
    //         $merchant_account->type         = 'income';
    //         $merchant_account->details      = 'parcel_total_delivery_charge_to_greenx_reverse_cause_delivery_reverse';
    //         $merchant_account->save();
    ...
    //     if(($current_status == "delivered" || $current_status == 'partially-delivered') &&
    //      ($status == 'received' || $status == 'delivery-assigned' || $status == 'transferred-received-by-branch' || $status == 'transferred-to-branch')){


    //         $this->deliveryTransactionReverse($id);

    //     }elseif(($current_status == "delivered" || $current_status == 'partially-delivered') && ($status == 'pending'
    //             || $status == 'pickup-assigned' || $status == 'received-by-pickup-man')){

    //         $this->deliveryTransactionReverse($id);
    //         $this->receivedTransactionReverse($id);

    //     }elseif(($current_status == "received" || $current_status == 'transferred-received-by-branch'
    //             || $current_status == 'transferred-to-branch' || $current_status == "delivery-assigned" || $current_status == "re-schedule-delivery")
    //         && ($status == 'pending' || $status == 'pickup-assigned' || $status == 'received-by-pickup-man')){

    //         $this->receivedTransactionReverse($id);

    //     }elseif(($current_status == "returned-to-merchant" && $parcel->is_partially_delivered) &&
    //      ($status == 'partially-delivered' || $status == 'returned-to-warehouse' || $status == 'return-assigned-to-merchant')){

    //         $this->returnTransactionReverse($id);

    //     }elseif(($current_status == "returned-to-merchant" && $parcel->is_partially_delivered) &&
    //      ($status == 'received' || $status == 'transferred-received-by-branch' || $status == 'transferred-to-branch'
    ...
    //     }elseif(($current_status == "returned-to-merchant") &&
    //      ($status == 'received' || $status == 'transferred-received-by-branch' || $status == 'transferred-to-branch'
    //          || $status == 'delivery-assigned' || $status == 'returned-to-warehouse' || $status == 'return-assigned-to-merchant')){
    ...
    //     }elseif(($current_status == "return-assigned-to-merchant" || $current_status == "returned-to-warehouse" || $current_status == "partially-delivered")
    ...
    //         $this->deliveryTransactionReverse($id);
    //         $this->receivedTransactionReverse($id);
    //     }elseif(($current_status == "return-assigned-to-merchant" || $current_status == "returned-to-warehouse") &&
    //     ($status == 'pending' || $status == 'pickup-assigned' || $status == 'received-by-pickup-man')){
    ...
    //     $delivery_account->source               = 'parcel_delivery';
    //     $delivery_account->amount               = $parcel->delivery_fee;
    //     $delivery_account->type                 = 'expense'; // previous expense
    ...
    //         if($parcel->status == 'received' || $parcel->status == 'transferred-to-branch'
    ...
    //         if($parcel->status_before_cancel == 'received' || $parcel->status_before_cancel == 'transferred-to-branch'
        ...
        if(($current_status == "delivered" || $current_status == 'partially-delivered') &&
         ($status == 'received' || $status == 'delivery-assigned' || $status == 'transferred-received-by-hub' || $status == 'transferred-to-hub')){
            ...
            $this->deliveryTransactionReverse($id);
                ...
                || $status == 'pickup-assigned' || $status == 'received-by-pickup-man')){
             //received, delivered all reversed
            //  dd('2');
...
//            }
        ...
        }elseif(($current_status == "received" || $current_status == 'transferred-received-by-hub'
...
//            }
        ...
        }elseif(($current_status == "return-assigned-to-merchant" || $current_status == "returned-to-branch" || $current_status == "partially-delivered")
            && $parcel->is_partially_delivered &&
            ($status == 'received' || $status == 'transferred-received-by-hub' || $status == 'transferred-to-hub'
              || $status == 'delivery-assigned')){
            //received reversed
...
//            }
            $this->deliveryTransactionReverse($id);
            $this->receivedTransactionReverse($id);
        }elseif(($current_status == "return-assigned-to-merchant" || $current_status == "returned-to-branch") &&
        ($status == 'pending' || $status == 'pickup-assigned' || $status == 'received-by-pickup-man')){
            //received reversed
...
//            }
    ...
    public function deliveryTransactionReverse($id){
        ...
        $merchant_account->source       = 'parcel_delivery';
        $merchant_account->amount       = $parcel->price;
        $merchant_account->type         = 'expense';  // previous income
        $merchant_account->details      = 'parcel_cod_collection_from_customer_reversed';
        $merchant_account->save();
    ...
    public function returnTransactionReverse($id){
        ...
        $merchant_account->amount    = $return_charge;
        $merchant_account->type      = 'income'; // previous expense
        $merchant_account->details   = __('parcel_return_fee_revers');
        $merchant_account->save();
    ...
    public function incomeExpenseManageCancel($id, $status){
        ...
        if($status == 'cancel'){
            ...
            if($parcel->status == 'received' || $parcel->status == 'transferred-to-hub'
            || $parcel->status == 'transferred-received-by-hub' || $parcel->status == 'delivery-assigned'
            || $parcel->status == 're-schedule-delivery' || $parcel->status == 'returned-to-branch'
            || $parcel->status == 'return-assigned-to-merchant')
            {
                $this->receivedTransactionReverse($id);
            }
            ...
            if($parcel->status_before_cancel == 'received' || $parcel->status_before_cancel == 'transferred-to-hub'
            || $parcel->status_before_cancel == 'transferred-received-by-hub' || $parcel->status_before_cancel == 'delivery-assigned'
            || $parcel->status_before_cancel == 're-schedule-delivery' || $parcel->status_before_cancel == 'returned-to-branch'
            || $parcel->status_before_cancel == 'return-assigned-to-merchant')
            {
                $this->incomeExpenseManage($id, 'received', 'received-reverse');
            }
    ...
    public function creditUpdate($request)
    {
        ...
        try{
            // get previous company_account record
            $company_account                       = CompanyAccount::find($request->id);
            if($this->IncomeLog($company_account->id, "company_account_update", $company_account)):
                $company_account->details              = $request->details;
                $company_account->date                 = date('Y-m-d', strtotime($request->date));
                $company_account->merchant_id          = $request->merchant;
                $company_account->amount               = $request->amount;
            ...
            // get previous staff_account record
            $staff_account                      = StaffAccount::where('company_account_id', $company_account->id)->orderByDesc('id')->first();
            if($this->IncomeLog($staff_account->id, "staff_account_update", $staff_account)):
                $staff_account->date                = date('Y-m-d', strtotime($request->date));
                $staff_account->amount              = $request->amount;
                $staff_account->user_id             = $company_account->user_id;
                $staff_account->account_id          = $request->account;
            ...
            //get previous merchant_account record
            $merchant_account                       = MerchantAccount::where('company_account_id', $company_account->id)->orderByDesc('id')->first();
            if($this->IncomeLog($merchant_account->id, "merchant_account_update", $merchant_account)):
                $merchant_account->details              = $request->details;
                $merchant_account->date                 = date('Y-m-d', strtotime($request->date));
                $merchant_account->amount               = $request->amount;
                $merchant_account->merchant_id          = $request->merchant;
    ...
    public function IncomeLog($id, $title, $data)
    {
        DB::beginTransaction();
        try{
            $log      = new AccountLog();
