...
trait UpdateTrait
{
    ...
    public function downloadUpdateFile($data): bool|string
    {
        ...
        try {
            $zip = new ZipArchive;
            $zipFilePath = "/var/www/delixfile.zip"; // Full path to the zip file in the base directory
            $extractToPath = base_path('/'); // Extract to the Laravel base path

            // Open the zip file
            if ($zip->open($zipFilePath) === TRUE) {
                // Extract to the base path
                $zip->extractTo($extractToPath);
                $zip->close();
            } else {
                return 'Zip file cannot be imported. Please check your server permissions or contact the script author.';
            }
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                return 'Error decoding config file: ' . json_last_error_msg();
            }

            // Update version settings
            $version_code         = $config['version_code'];
            $version_title        = $config['version_title'];
            ...
            $this->updateSetting('version_code', $version_code);
            $this->updateSetting('version_title', $version_title);

            Artisan::call('all:clear');
            return true;

        } catch (\Exception $e) {
            return 'An error occurred: ' . $e->getMessage(); // Return error message
        }
    }

    // Helper function to update settings
    private function updateSetting(string $title, string $value): void
    {
        $setting = Setting::where('title', $title)->first();
        if ($setting) {
            $setting->update(['value' => $value]);
        } else {
            Setting::create(['title' => $title, 'value' => $value]);
        }
    }
