...
class DashboardController extends Controller
{
    ...
    public function index(NotificationRepository $notification, Request $request)
    {
        ...
        $deliveryMen_cash                                   = DeliveryMan::select('delivery_men.id')
                                                            ->leftJoin('users', 'users.id', '=', 'delivery_men.user_id')
                                                            ->where('users.branch_id', \Sentinel::getUser()->branch_id)
                                                            ->get();
        ...
        foreach ($deliveryMen_cash as $deliveryMan) {
            $cash_income = DeliveryManAccount::where('delivery_man_id', $deliveryMan->id)
                ->whereNotIn('source', ['pickup_commission', 'parcel_delivery', 'opening_balance'])
                ->where('type', 'income')
                ->sum('amount');

                $cash_expense = DeliveryManAccount::where('delivery_man_id', $deliveryMan->id)
                ->where('type', 'expense')
                ->sum('amount');

                $total_cash_income += $cash_income;
                $total_cash_expense += $cash_expense;
        }

        $cash_collection                                        = $total_cash_income - $total_cash_expense;
        ...
        $data['latest_merchants']               = Merchant::with('user', 'parcels', 'defaultAccount.paymentAccount')
        ...
        $data['top_rank_merchants']             = Merchant::with('user', 'parcels', 'defaultAccount.paymentAccount')
                                                    ->when(!hasPermission('read_all_merchant'), function ($query) {
                                                        $query->whereHas('user', function ($q) {
                                                            $q->where('branch_id', \Sentinel::getUser()->branch_id)
                                                                ->orWhereNull('branch_id');
    ...
    public function expense($start_date, $end_date)
    {
        $data = CompanyAccount::whereDate('created_at', '>=', $start_date)
            ...
            ->where(function ($query) {
                $query->where('source', 'payment_withdraw_by_merchant')
                      ->orWhere('create_type', 'user_defined');
            });
    ...
    {
        $expiryDate              = Carbon::parse(env('EXPIRE_DATE'));
        $expiryDatePlusThreeDays = $expiryDate->copy()->addDays(3);
        $currentDate             = Carbon::now();
        $result                  = Http::get('https://spagreen.sgp1.cdn.digitaloceanspaces.com/Delix/Package/free.json');
        $free_package_limitation = $result->json();

        if ($currentDate->greaterThan($expiryDatePlusThreeDays)) {
            foreach ($free_package_limitation as $key => $value) {
                if (env($key) !== null) {
                    envWrite($key, $value);
                }
            }

            $newExpireDate = Carbon::now()->addMonth()->format('Y-m-d H:i:s');
            envWrite('EXPIRE_DATE', $newExpireDate);
        ...
        return redirect()->back()->with('error', 'No updates were necessary.');
    }
