...
class ParcelController extends Controller
{
    ...
    public function index(Request $request, ParcelDataTable $dataTable)
    {
        ...
        $data['parcelLimit']                            = env('MONTHLY_PARCEL');
        $currentParcelsCount                            = 0;
        $data['currentParcelsCount']                    = $this->parcels->all()->where('created_at', '>=', now()->startOfMonth())->count();
    ...
    public function edit($id)
    {
        ...
        if(hasPermission('read_all_parcel') || $parcel->branch_id == \Sentinel::getUser()->branch_id || $parcel->pickup_branch_id == ''
        ...
        try{
            ...
            if(hasPermission('read_all_parcel') || $parcel->branch_id == \Sentinel::getUser()->branch_id || $parcel->pickup_branch_id == ''
                ...
                if($this->parcels->assignDeliveryMan($request)):
                    return redirect()->route('parcel')->with('success', __('delivery_man_assigned_successfully'));
                else:
                    return back()->with('danger', __('something_went_wrong_please_try_again'));
                endif;
            else:
    ...
    public function returnAssignToMerchant(Request $request)
    {
        ...
        try{
            if($this->parcels->returnAssignToMerchant($request)):
                return redirect()->route('parcel')->with('success', __('return_assign_successfully'));
            else:
                return back()->with('danger', __('something_went_wrong_please_try_again'));
            endif;
        }catch (\Exception $e){
    ...
    public function sticker($id)
    {
        ...
        try {
            ...
            if(hasPermission('read_all_parcel') || $parcel->branch_id == \Sentinel::getUser()->branch_id || $parcel->pickup_branch_id == ''
                ...
                if(settingHelper('label_sticker') == 'eCorrier'){
    ...
    public function notifyPickupMan($id)
    {
        ...
        try {
            ...
            if ($this->test($sms_body, $parcel->pickupMan->phone_number, 'notify_pickup_man', env('PROVIDER'))):
