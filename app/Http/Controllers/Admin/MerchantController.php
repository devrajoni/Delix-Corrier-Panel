...
class MerchantController extends Controller
{
    ...
    public function store(MerchantStoreRequest $request)
    {
        ...
        try {
            ...
            if ($current_merchant_count >= $merchant_limit) :
                return redirect()->route('merchant')->with('danger', __('merchant_creation_limit_exceeded'));
            endif;
    ...
    public function statusChange(Request $request)
    {
        ...
        try {
            $merchantLimit                       = env('ACTIVE_MERCHANT');
            $currentMerchantCount                = 0;
            if($request->status ==1):
                $currentMerchantCount            = Merchant::where('status', 1)->where('created_at', '>=', now()->startOfMonth())->count();
                if ($currentMerchantCount >= $merchantLimit):
                    return response()->json([
                        'status' =>404,
                        'message'=> __('active_merchant_limit_exceeded'),
                    ]);
                endif;
            endif;
