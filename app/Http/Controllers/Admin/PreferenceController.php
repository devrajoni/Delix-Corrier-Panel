...
class PreferenceController extends Controller
{
    ...
    public function smsSetting(): \Illuminate\Contracts\View\Factory|\Illuminate\Contracts\View\View|\Illuminate\Contracts\Foundation\Application
    {
    ...
    public function emailSetting()
    {
        ...
        $customer_sms_templates = CustomerParcelSmsTemplates::all();
        $withdraw_sms_templates = WithdrawSmsTemplate::all();
        return view('admin.email-setup.email-template', compact('sms_templates','customer_sms_templates','withdraw_sms_templates'));
    }

    public function smsStatusUpdate()
    {
        try {
            $sms_histories      = SMSHistory::where('status', 'approved')->orWhere('status', 'pending')->get();

            foreach ($sms_histories as $sms_history):
                if (!empty($sms_history->message_id)):
                    $apiKey         = env('SPAGREEN_SMS_KEY');
                    $secretKey      = env('SPAGREEN_SECRET_KEY');
                    $message_id     = $sms_history->message_id;

                    $response       = Http::get("http://apismpp.revesms.com/getstatus", [
                        'apikey'    => $apiKey,
                        'secretkey' => $secretKey,
                    ...
                    if ($response->successful()):
                        $check_status               = json_decode($response->body());

                        if (isset($check_status->Text)):
                            $sms_history->status    = strtolower($check_status->Text);
                            $sms_history->save();

                            if ($sms_history->status == 'rejectd'):
                                $sms_calculate      = Setting::where('title', 'total_usages')->first();

                                if ($sms_calculate):
                                    $sms_calculate->value   -= 1;
                                else:
                                    $sms_calculate          = new Setting();
                                    $sms_calculate->title   = 'total_usages';
                                    $sms_calculate->value   = 0;
                                endif;
