...
class UserController extends Controller
{
    ...
    public function index(StaffDataTable $dataTable, Request $request)
    {
        ...
        $data['staff_limit']                               = env('ACTIVE_STAFF');
        $data['current_staff_count']                       = 0;
        $data['current_staff_count']                       = User::where('status', 'active')->where('is_super_admin', '!=', '1')->where('is_admin', '!=', '1')->where('status', 'active')->where('user_type', 'staff')->where('created_at', '>=', now()->startOfMonth())->count();
    ...
    public function store(UserStoreRequest $request)
    {
        ...
        try{
            $staff_limit                               = env('ACTIVE_STAFF');
            $current_staff_count                       = 0;
            $current_staff_count                       = User::where('status', 'active')->where('is_super_admin', '!=', '1')->where('is_admin', '!=', '1')->where('user_type', 'staff')->where('created_at', '>=', now()->startOfMonth())->count();
            if ($current_staff_count >= $staff_limit):
                return redirect()->route('users')->with('danger', __('staff_create_limit_exceeded'));
            endif;
    ...
    public function update(UserUpdateRequest $request)
    {
        ...
        try{
            ...
            if($this->users->update($request->all())):
    ...
    public function statusChange(Request $request)
    {
        ...
        try{
            $staffLimit                              = env('ACTIVE_STAFF');
            $currentStaffCount                       = 0;
            if($request->status ==1):
                $currentStaffCount                   = User::where('status', 'active')->where('is_super_admin', '!=', '1')->where('is_admin', '!=', '1')->where('user_type', 'staff')->where('created_at', '>=', now()->startOfMonth())->count();
                if ($currentStaffCount >= $staffLimit):
                    return response()->json([
                        'status' =>404,
                        'message'=> __('active_staff_limit_exceeded'),
                    ]);
                endif;
            endif;
