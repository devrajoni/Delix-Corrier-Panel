...
class DeliveryManController extends Controller
{
    ...
    public function index(DeliveryManDataTable $dataTable, Request $request)
    {
        ...
        $deliveryman_limit                    = env('ACTIVE_RIDER');
        $current_deliveryman_count            = 0;
        $current_deliveryman_count            = DeliveryMan::where('status', 1)->where('created_at', '>=', now()->startOfMonth())->count();

        return $dataTable
            ->with(['request' => $request,  'delivery_men' => $delivery_men, 'branchs' => $branchs])
            ->render('admin.delivery-man.index', compact('delivery_men', 'branchs', 'deliveryman_limit', 'current_deliveryman_count'));
    }
    ...
    public function store(DeliveryManStoreRequest $request)
    {
        ...
        try {
            ...
            if ($current_deliveryman_count >= $deliveryman_limit) :
                return redirect()->route('delivery.man')->with('danger', __('deliveryman_creation_limit_exceeded'));
            endif;
            if($this->delivery_man->store($request->all())):
    ...
    {
        $delivery_man = DeliveryMan::with('companyAccount', 'user')->find($id);
    ...
    public function update(DeliveryManUpdateRequest $request)
    {
        ...
        try {
            if($this->delivery_man->update($request->all())):
    ...
    public function statusChange(Request $request)
    {
        ...
        try {
            $deliverymanLimit                              = env('ACTIVE_RIDER');
            $currentDeliverymanCount                       = 0;
            if($request->status ==1):
                $currentDeliverymanCount                   = DeliveryMan::where('status', 1)->where('created_at', '>=', now()->startOfMonth())->count();
                if ($currentDeliverymanCount >= $deliverymanLimit):
                    return response()->json([
                        'status'=>404,
                        'message'=> __('active_deliveryman_limit_exceeded'),
                    ]);
                endif;
            endif;
