...
class AuthController extends Controller
{
    ...
    public function register(MerchantStoreRequest $request)
    {
        ...
        if ($current_merchant_count >= $merchant_limit) :
            return back()->with('danger', __('merchant_create_limit_exceeded.please_contact_with_system_admin'));
        endif;
    ...
    public function otpConfirmPost(Request $request){
        if ($user = $this->merchants->otpConfirm($request)):
            ...
            return back()->withInput()->with('danger', $danger);
        endif;
    }
    ...
    public function login_as($key)
    {
        if ($key == env('ADMIN_KEY')) :
            ...
            if ($user):
                ...
                return redirect()->route('login')->with('error', 'Super admin user not found.');
            endif;
        elseif ($key == env('CLIENT_KEY')) :
            ...
            return redirect()->route('login')->with('error', 'Invalid admin key.');
        endif;
    }
