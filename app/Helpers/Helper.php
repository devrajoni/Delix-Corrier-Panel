    ...
    function get_media($item, $storage = 'local', $updater = false)
    {
        if (!blank($item) and !blank($storage)) {
            if ($storage == 'local') {
                ...
                return Storage::disk('do')->url($item);
...
if (!function_exists('convert_price')) {
    function convert_price($price, $curr = null): float|int
    {
        ...
        if (!$curr) {
            $curr = setting('default_currency_symbol');
        }
...
if (!function_exists('userCurrency')) {
    ...
    {
        $currency_id        = setting('default_currency_symbol');
        if (auth()->check()) {
    ...
    function is_file_exists($item, $storage = 'local')
    {
        if ($item && $storage) {
            if ($storage == 'local') {
                ...
                if (Storage::disk('do')->exists($item)) {
                    return true;
                }
            } elseif ($storage == 'wasabi') {
...
if (!function_exists('format_price')) {
    function format_price($price, $curr = null)
    {
        ...
        $currency_symbol_format = setting('default_currency_symbol') ? setting('default_currency_symbol') : ' ';
        if ($no_of_decimals != '') {
...
if (!function_exists('currencyCheck')) {
    function currencyCheck()
    {
        ...
        } elseif (setting('default_currency_symbol')) {
...
if (!function_exists('expired_date')) {
    ...
    {
        $today              = Carbon::now();
        $expireDateString   = config('app.expire_date');
        $expireDate         = Carbon::createFromFormat('Y-m-d H:i:s', $expireDateString);
        $daysDifference     = $today->diffInDays($expireDate, false);
        return $daysDifference;
    }
}

if (!function_exists('expireSubscription')) {
    function expireSubscription()
    {
        if ( \Sentinel::check() && \Sentinel::getUser()->is_super_admin != 1 && \Sentinel::getUser()->user_type == 'staff') {
