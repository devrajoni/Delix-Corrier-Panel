...
class MerchantDatatable extends DataTable
{
    ...
    public function query(Merchant $model): QueryBuilder
    {
        ...
        $query = $model->with('user', 'parcels', 'defaultAccount.paymentAccount')
            ...
            ->when($this->request->company_name, function ($query, $search) {
             ...
             ->when($this->request->has('approval_status') && $this->request->approval_status !== "", function ($query) {
                $approve = $this->request->approval_status;
                $query->where('registration_confirmed', $approve);
            })

            ->when($this->request->has('sort_by') && $this->request->sort_by !== "", function ($query) {
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::computed('company_staff')->title(__('company_staff')),
            Column::computed('payment')->title(__('payment')),
            Column::computed('parcels')->title(__('parcels')),
            Column::computed('status')->addClass('text-center status_fix')->title(__('status')),
