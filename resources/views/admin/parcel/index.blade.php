...
@section('mainContent')
                    ...
                    <div class="oftions-content-right mb-12 filterOPT">
                        ...
                        @if(hasPermission('parcel_create'))
                            @if(@settingHelper('preferences')->where('title','create_parcel')->first()->staff)
                                <a href="{{route('import.csv')}}" class="d-flex align-items-center btn btn-sm sg-btn-primary gap-2 @if($currentParcelsCount >= $parcelLimit) disabled @endif"><i class="icon las la-plus"></i><span>{{__('import')}}</span></a>
                            @endif
                        @endif

                        @if(hasPermission('parcel_transfer_to_branch'))
                            <a href="{{route('bulk.transfer')}}" class="d-flex align-items-center btn btn-sm sg-btn-primary gap-2"><i class="icon las la-plus"></i><span>{{__('branch_transfer')}}</span></a>
                        @endif
                        @if(hasPermission('parcel_transfer_receive_to_branch'))
                            <a href="{{route('bulk.transfer.receive')}}" class="d-flex btn-sm align-items-center btn sg-btn-primary gap-2"><i class="icon las la-plus"></i><span>{{__('transfer_receive')}}</span></a>
                        @endif
                        @if(hasPermission('parcel_pickup_assigned'))
                            <a href="{{route('bulk.pickup.assign')}}" class="d-flex align-items-center btn btn-sm sg-btn-primary gap-2"><i class="icon las la-plus"></i><span>{{__('assign_pickup')}}</span></a>
                        @endif
                        @if(hasPermission('parcel_delivery_assigned'))
                            <a href="{{route('bulk.assigning')}}" class="d-flex align-items-center btn btn-sm sg-btn-primary gap-2"><i class="icon las la-plus"></i><span>{{__('assign_delivery')}}</span></a>
                        @endif
                        @if(hasPermission('parcel_create'))
                            @if(@settingHelper('preferences')->where('title','create_parcel')->first()->staff)
                                <a href="{{route('parcel.create')}}" class="d-flex align-items-center btn btn-sm sg-btn-primary gap-2 @if($currentParcelsCount >= $parcelLimit) disabled @endif"><i class="icon las la-plus"></i><span>{{__('create').' '.__('parcel')}}</span></a>
                            @else
                                <button class="d-flex align-items-center btn btn-sm sg-btn-primary gap-2"><i class="icon las la-plus"></i><span>{{__('create').' '.__('parcel').' ('.__('service_unavailable').')'}}</span></button>
                            @endif
                        @endif
                    </div>
                </div>
                        ...
                        <div class="hidden-filter bg-white redious-border p-20 p-sm-30 mb-4">
                                <div class="row gx-6 gy-3">
                                    <div class="col-3">
                                        ...
                                        <select id="merchant_id" name="merchant_id" class="form-control select-merchant merchant merchant-live-search filterable" data-url="{{ route('get-merchant-live') }}">
                                            <option value="">{{ __('select_merchant') }}</option>
                                        </select>
                                    </div>
                                    <div class="col-3">
                                        <label class="form-label" for="phone_number">{{ __('phone_number') }}</label>
                                        <input type="text" class="form-control filterable" id="phone_number" name="phone_number" placeholder="{{__('enter').' '.__('company').' '.__('phone_number')}}">
                                    </div>
                                    <div class="col-3">
                                        <label class="form-label" for="customer_invoice_no">{{ __('customer_invoice_no') }}</label>
                                        <input type="text" class="form-control filterable" id="customer_invoice_no" name="customer_invoice_no" placeholder="{{__('enter').' '.__('invoice_no')}}">
                                    </div>
                                    <div class="col-3">
                                        <label class="form-label" for="created_at">{{ __('created_at') }}</label>
                                        <input type="text" name="created_at" id="created_at" class="form-control date-range filterable" placeholder="YYYY-MM-DD to YYYY-MM-DD" id="created_from">
                                    </div>
                                    <div class="col-3">
                                        <label class="form-label" for="pickup_date">{{ __('pickup_date') }}</label>
                                        <input type="text" name="pickup_date" id="pickup_date" class="form-control date-picker filterable" placeholder="YYYY-MM-DD" id="pickup_date">
                                    </div>
                                    <div class="col-3">
                                        <label class="form-label" for="delivery_date">{{ __('delivery_date') }}</label>
                                        <input type="text" name="delivery_date" id="delivery_date" class="form-control date-picker filterable" placeholder="YYYY-MM-DD" id="delivery_date">
                                    </div>
                                    <div class="col-3">
                                        <label class="form-label" for="delivered_date">{{ __('delivered_date') }}</label>
                                        <input type="text" name="delivered_date" id="delivered_date" class="form-control date-picker filterable" placeholder="YYYY-MM-DD" id="delivered_date">
                                    </div>

                                    <div class="col-3">
                                        <label class="form-label" for="pickup_man_id">{{ __('pickup_man') }}</label>
                                        <select name="pickup_man_id" id="pickup_man_id" class="form-control filterable" data-url="{{ route('get-delivery-man-live') }}">
                                            <option value="">{{__('pickup_man')}}</option>
                                        </select>
                                    </div>
                                    <div class="col-3">
                                        <label class="form-label" for="delivery_man_id">{{ __('delivery_man') }}</label>
                                        <select name="delivery_man_id" id="delivery_man_id" class="form-control  filterable" data-url="{{ route('get-delivery-man-live') }}">
                                            <option value="">{{__('delivery_man')}}</option>
                                        </select>
                                    </div>
                                    <div class="col-3">
                                        <label class="form-label" for="third_party">{{ __('third_party') }}</label>
                                        <select name="third_party_id" id="third_party" class="without_search form-select form-control form-select-sm filterable" >
                                    ...
                                    <div class="col-3">
                                        ...
                                        <select class="without_search form-select form-control form-select-sm filterable" id="status" name="status">
                                    ...
                                    <div class="col-3">
                                        ...
                                        <select class="without_search form-select form-control form-select-sm filterable" id="any_weight" name="weight">
                                    ...
                                    <div class="col-3">
                                        ...
                                        <select class="without_search form-select form-control form-select-sm filterable" id="parcel_type" name="parcel_type">
                                    ...
                                    <div class="col-3">
                                        ...
                                        <select class="without_search form-select form-control form-select-sm filterable" id="location" name="location">
                                    ...
                                    <div class="col-3">
                                        ...
                                        <select class="without_search form-select form-control form-select-sm filterable" id="branch_id" name="branch_id">
                                    ...
                                    <div class="col-3">
                                        ...
                                        <select class="without_search form-select form-control form-select-sm filterable" id="pickup_branch_id" name="pickup_branch_id">
                                    ...
                                    <div class="col-12 text-right">
                                        <div class="d-flex justify-content-end gap-2">
                                            <div class="mb-3">
                                                <button type="button" id="download"  class="btn sg-btn-primary">{{__('download')}}</button>
                                            </div>
                                            <div class="mb-3">
                                                <button type="submit" id="filter" class="btn sg-btn-primary">{{__('filter')}}</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    @include('admin.parcel.modal.create-paperflyparcel')
